...
`1.5.2-beta.4` clamps each message to the fitted line count, uses one bounded rendered verification before a new fit becomes trusted in cache, biases Lite Canvas width prediction conservatively, and reserves additional line-box headroom. The fixed bottom band can therefore ellipsize safely within its own bounds rather than letting a late wrapped line fall below the viewport.
