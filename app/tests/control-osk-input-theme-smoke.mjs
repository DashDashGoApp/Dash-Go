...
assert.match(consistency,/#ctrl \.oskfield,[\s\S]*?min-height:52px;[\s\S]*?background:var\(--panel\);[\s\S]*?border-radius:12px;/,"OSK inputs require one complete Control surface independent of page position");
...
assert.match(mapCss,/#mapsearch\{[\s\S]*?min-height:52px;[\s\S]*?border-radius:12px;[\s\S]*?font:700 17px\/1\.2 var\(--sans\)/,"map search must share the Control text-input geometry");
...
assert.match(consistency,/--ctrl-danger-fg:var\(--sun\);[\s\S]*?--ctrl-danger-armed-bg:/,"danger state must be defined through one token family");
